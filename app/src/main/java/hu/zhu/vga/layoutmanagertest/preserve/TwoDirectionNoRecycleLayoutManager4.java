...
import hu.zhu.vga.layoutmanagertest.TestViewHolder;

/**
 * Created by Zhuinden on 2015.11.05..
 */
public class TwoDirectionNoRecycleLayoutManager4
        extends RecyclerView.LayoutManager {
    class RowSpecifierAdapter
    ...
    //@formatter:on
        ...
        public int getAccumulatedWidthForPosition(int position) {
            if(!positionToRowAndAccumulatedWidthMap.containsKey(position)) {
                initializeMetadataForPosition(position);
            }
            return positionToRowAndAccumulatedWidthMap.get(position).getAccumulatedWidth();
        }
    ...
    public TwoDirectionNoRecycleLayoutManager4(RowSpecifierAdapter rowSpecifierAdapter) {
    ...
    private boolean isVisible(int position) {
        ensureAnchorInfo();
        int accumulatedWidth = rowSpecifierAdapter.getAccumulatedWidthForPosition(position);
        if(accumulatedWidth >= anchorInfo.currentScrolledX && accumulatedWidth < (anchorInfo.currentScrolledX + ScreenUtils.getScreenWidth())) {
            return true;
        } else {
            return false;
        }
    ...
    private void fill(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        layoutState.reset();
        ensureAnchorInfo();
        layoutState.rowCount = rowSpecifierAdapter.getRowNumber(anchorInfo.currentScrolledY);
        viewCache.clear();
        ...
        for(Integer i : visiblePositions) {
            ...
            if(view == null) {
                ...
                //Log.i(getClass().getSimpleName(), "[" + i + "] Measured Width and Height: [" + decoratedMeasuredWidth + "|" + decoratedMeasuredHeight + "]");
                int previousRow = layoutState.rowCount;
                if(rowSpecifierAdapter.getRowCountForPosition(i) > previousRow) {
                    //layoutState.columnCount = 0;
                    layoutState.rowCount = rowSpecifierAdapter.getRowCountForPosition(i);
